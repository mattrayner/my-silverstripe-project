	...
	task :build do
		on roles(:web) do
			within fetch(:deploy_to) do
				execute :if, "test ! -d shared/silverstripe-cache/*; then echo \"Directory does not exist 'shared/silverstripe-cache/*'\" 1>&2; else rm -R shared/silverstripe-cache/*; fi "
				# Need to figure out issues with flushing the cache before enabling curl
				execute :curl, "-v -L #{fetch(:curl_command)}"
			end
